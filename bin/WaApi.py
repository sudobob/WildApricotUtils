...
"""
From: https://github.com/WildApricot/ApiSamples/blob/master/python/WaApi.py
...
class WaApiClient(object):
    ...
    def execute_request(self, api_url, api_request_object=None, method=None):
        ...
        if not api_url.startswith("http"):
            api_url = f'{self.api_endpoint}{api_url}'
...
class ApiObject(object):
    ...
    def to_dict(self):
        new_dict = {}
        for key, value in vars(self).items():
            if isinstance(self.__dict__[key], ApiObject):
                new_dict[key] = value.to_dict()
            elif isinstance(value, list):
                new_list = []
                for list_item in value:
                    if isinstance(list_item, ApiObject):
                        new_list.append(list_item.to_dict())
                    else:
                        new_list.append(list_item)
